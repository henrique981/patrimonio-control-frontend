...
const Badge = ({ status }) => {
  const map = {
    ...
    inservivel:         { color: '#ef4444', bg: '#ef444420', label: 'Inservível' },
    descarga:           { color: '#f59e0b', bg: '#f59e0b20', label: 'Descarga' },
    inexistente:        { color: '#8b5cf6', bg: '#8b5cf620', label: 'Inexistente' },
    em_manutencao:      { color: '#06b6d4', bg: '#06b6d420', label: 'Manutenção' },
    extraviado:         { color: '#ef4444', bg: '#ef444420', label: 'Extraviado' },
    aguardando_descarga:{ color: '#f59e0b', bg: '#f59e0b20', label: 'Ag. Descarga' },
    aguardando_liberacao:{ color: '#f59e0b', bg: '#f59e0b20', label: 'Ag. Liberação' },
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <button onClick={onClose} style={{
          ...
          width: 32, height: 32, borderRadius: 8, fontSize: 18, lineHeight: 1
        }}>×</button>
      </div>
      {children}
    </div>
...
const Table = ({ columns, data, onEdit, onDelete }) => (
  <div style={{ overflowX: 'auto' }}>
            ...
            <td style={{ padding: '10px 14px' }}>
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={() => onEdit(row)} style={{
                  background: '#3b82f620', color: 'var(--accent)',
                  border: '1px solid #3b82f640', padding: '4px 10px', fontSize: 12
                }}>Editar</button>
                <button onClick={() => onDelete(row)} style={{
                  background: '#ef444420', color: 'var(--red)',
                  border: '1px solid #ef444440', padding: '4px 10px', fontSize: 12
                }}>Excluir</button>
              </div>
            </td>
          </tr>
    ...
    {data.length === 0 && (
      <div style={{ textAlign: 'center', padding: 40, color: 'var(--text3)' }}>Nenhum registro encontrado</div>
    )}
  </div>
);
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
        {[
          ...
          <div key={title} style={{
            ...
            <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text2)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 16 }}>{title}</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {data.map(item => (
                <div key={item.situacao} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Badge status={item.situacao} />
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, fontWeight: 700 }}>{item.total}</span>
                </div>
              ))}
            </div>
          </div>
...
const TabItens = ({ showToast }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) { showToast('Item excluído!', 'success'); setModal(null); load(); }
  ...
  const cols = [
    { key: 'patrimonio', label: 'Patrimônio', mono: true },
    { key: 'nome_material', label: 'Material' },
    { key: 'n_serie', label: 'Nº Série' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        <input placeholder="🔍  Buscar por nome, patrimônio ou série..." value={search} onChange={e => setSearch(e.target.value)} style={{ flex: 1, minWidth: 240 }} />
      ...
      {(modal === 'add' || modal === 'edit') && (
        <Modal title={modal === 'add' ? 'Novo Item' : 'Editar Item'} onClose={() => setModal(null)}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <FormField label="Patrimônio" name="patrimonio" value={form.patrimonio} onChange={handleChange} required span={modal === 'add' ? 1 : 2} />
            {modal === 'add' && <FormField label="Nº Série" name="n_serie" value={form.n_serie} onChange={handleChange} />}
            <FormField label="Nome do Material" name="nome_material" value={form.nome_material} onChange={handleChange} required span={2} />
            <FormField label="Especificações" name="especificacoes" value={form.especificacoes} onChange={handleChange} span={2} />
            ...
            <FormField label="Situação" name="situacao" value={form.situacao} onChange={handleChange} options={SITUACOES_ITEM} required />
            ...
            <FormField label="Responsável RE" name="responsavel_re" value={form.responsavel_re} onChange={handleChange} />
            <FormField label="Responsável Nome" name="responsavel_nome" value={form.responsavel_nome} onChange={handleChange} />
            <FormField label="Observação" name="observacao" value={form.observacao} onChange={handleChange} span={2} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20 }}>
            <button onClick={() => setModal(null)} style={{ background: 'var(--border)', color: 'var(--text2)', padding: '9px 20px' }}>Cancelar</button>
            <button onClick={handleSave} disabled={saving} style={{ background: 'var(--accent)', color: '#fff', padding: '9px 20px', opacity: saving ? 0.7 : 1 }}>
      ...
      {modal === 'delete' && (
        <Modal title="Confirmar Exclusão" onClose={() => setModal(null)}>
          <p style={{ color: 'var(--text2)', marginBottom: 20 }}>
            Deseja excluir o item <strong style={{ color: 'var(--text)' }}>{selected?.nome_material}</strong> (patrimônio <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--cyan)' }}>{selected?.patrimonio}</span>)?
          </p>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            <button onClick={() => setModal(null)} style={{ background: 'var(--border)', color: 'var(--text2)', padding: '9px 20px' }}>Cancelar</button>
            <button onClick={handleDelete} disabled={saving} style={{ background: 'var(--red)', color: '#fff', padding: '9px 20px', opacity: saving ? 0.7 : 1 }}>
...
const TabArmas = ({ showToast }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) { showToast('Arma excluída!', 'success'); setModal(null); load(); }
      else showToast(res.erro || 'Erro ao excluir', 'error');
    } catch { showToast('Erro de conexão', 'error'); }
    setSaving(false);
  };
  ...
  const cols = [
    ...
    { key: 'n_serie', label: 'Nº Série', mono: true },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        <input placeholder="🔍  Buscar por nome, patrimônio, série ou RE..." value={search} onChange={e => setSearch(e.target.value)} style={{ flex: 1, minWidth: 240 }} />
        <select value={filtroSituacao} onChange={e => setFiltroSituacao(e.target.value)} style={{ minWidth: 160 }}>
          <option value="">Todas situações</option>
          {SITUACOES_ITEM.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
        </select>
      ...
      {(modal === 'add' || modal === 'edit') && (
        <Modal title={modal === 'add' ? 'Nova Arma' : 'Editar Arma'} onClose={() => setModal(null)}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <FormField label="Patrimônio" name="patrimonio" value={form.patrimonio} onChange={handleChange} required />
            <FormField label="Nº Série" name="n_serie" value={form.n_serie} onChange={handleChange} />
      ...
      {modal === 'delete' && (
        <Modal title="Confirmar Exclusão" onClose={() => setModal(null)}>
          <p style={{ color: 'var(--text2)', marginBottom: 20 }}>
            Deseja excluir <strong style={{ color: 'var(--text)' }}>{selected?.nome_material}</strong> — série <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--cyan)' }}>{selected?.n_serie}</span>?
          </p>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            <button onClick={() => setModal(null)} style={{ background: 'var(--border)', color: 'var(--text2)', padding: '9px 20px' }}>Cancelar</button>
            <button onClick={handleDelete} disabled={saving} style={{ background: 'var(--red)', color: '#fff', padding: '9px 20px', opacity: saving ? 0.7 : 1 }}>
...
const TabViaturas = ({ showToast }) => {
  ...
  const openAdd = () => { setForm({ situacao: 'operacional', unidade_opm: '606065000', combustivel: 'gasolina', km_atual: 0 }); setModal('add'); };
  const openEdit = row => { setForm({ ...row }); setSelected(row); setModal('edit'); };
  const openDelete = row => { setSelected(row); setModal('delete'); };
  const handleChange = e => setForm(f => ({ ...f, [e.target.name]: e.target.value }));
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) { showToast('Viatura excluída!', 'success'); setModal(null); load(); }
      else showToast(res.erro || 'Erro ao excluir', 'error');
    } catch { showToast('Erro de conexão', 'error'); }
    setSaving(false);
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {(modal === 'add' || modal === 'edit') && (
        <Modal title={modal === 'add' ? 'Nova Viatura' : 'Editar Viatura'} onClose={() => setModal(null)}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            ...
            <FormField label="Ano Fabricação" name="ano_fabricacao" value={form.ano_fabricacao} onChange={handleChange} type="number" />
            ...
            <FormField label="Combustível" name="combustivel" value={form.combustivel} onChange={handleChange} options={[{ value: 'gasolina', label: 'Gasolina' }, { value: 'etanol', label: 'Etanol' }, { value: 'flex', label: 'Flex' }, { value: 'diesel', label: 'Diesel' }]} />
            <FormField label="KM Atual" name="km_atual" value={form.km_atual} onChange={handleChange} type="number" />
            <FormField label="Situação" name="situacao" value={form.situacao} onChange={handleChange} options={SITUACOES_VTR} required />
      ...
      {modal === 'delete' && (
        <Modal title="Confirmar Exclusão" onClose={() => setModal(null)}>
          <p style={{ color: 'var(--text2)', marginBottom: 20 }}>
            Deseja excluir a viatura <strong style={{ color: 'var(--text)' }}>{selected?.prefixo}</strong>?
          </p>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            <button onClick={() => setModal(null)} style={{ background: 'var(--border)', color: 'var(--text2)', padding: '9px 20px' }}>Cancelar</button>
            <button onClick={handleDelete} disabled={saving} style={{ background: 'var(--red)', color: '#fff', padding: '9px 20px', opacity: saving ? 0.7 : 1 }}>
        ...
        <nav style={{ display: 'flex', gap: 4 }}>
          {abas.map(a => (
            <button key={a.id} onClick={() => setAba(a.id)} style={{
              ...
              border: aba === a.id ? 'none' : '1px solid transparent',
            }}>{a.label}</button>
          ))}
        </nav>
        <div style={{ flex: 1 }} />
